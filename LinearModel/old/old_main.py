...
for ep in range(0,episodes):
    ...
    if ep > pre_train:
        ...
        std_loss = std_a**2
        E_dr_dstd_a = MBDPG_std_w * torch.autograd.grad(std_loss, std_a, retain_graph=True)[0]
        ...
        with torch.no_grad():
            # Mean action grad 
            R_dr_dmu_a = (-1/(2*action_std**2) * (action - mu_a)**2) * rwd 
            # Std action grad
            R_dr_dstd_a = Reif_std_w * ((-1) * rwd * (action_std**2 - (action - mu_a)**2) / action_std**3)
        #Combine two grads relative to mu and std into one vector
