...
axs[0,0].errorbar(conditions,ataxia_mean[0], yerr=ataxia_se[0], ls='none', color='black',  elinewidth=1, capsize=1.5, alpha=alpha) # ecolor='lightslategray',
...
#axs[00].set_xlabel('CB contribution')
...
for m,s in zip(ataxia_mean, ataxia_se):
    if i ==0:
        ...
        axs[0,1].errorbar(updt_conditions[i,2],m[2], yerr=s[2], capsize=3, fmt="r--o",markerfacecolor=colors[2],markeredgecolor=colors[2],c='black',elinewidth=1,alpha=alpha)
    i+=1
...
axs[0,2].plot(t,RBL_grad,label='RBL',c=colors[0],alpha=alpha)
...
axs[0,3].plot(t,EBL_grad,label='EBL',c=colors[2],alpha=alpha)
