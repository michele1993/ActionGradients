...
class Actor(nn.Module):
    ...
    def __init__(self,input_s=1,action_s=1,ln_rate = 1e-3, trainable = True, opt_type='Adam'):
        ...
        # Initialise std to be large since target unknown
        self.small_weight_init(self.l1) 
        self.std_weight_init(self.l2) 
    ...
    def std_weight_init(self,l):
        """ Initialise log(std) weight to mean negative value to avoid massive values when taking exp() """
        if isinstance(l,nn.Linear):
            nn.init.normal_(l.weight,mean=-1,std= 0.1)# std= 0.00005
